...
def modify_context_task(new_context , id, filename):
  ...
  with open(filename, 'w') as lestaches:
    ...
    for line in lines:
      ...
      current_description= line[1:line.find(current_context)]
      if current_id == id:
        search = True
        # si l'id est trouvé on écrit la nouvelle description
        joined_string = " ".join(new_context)
        lestaches.write(f"{id} {current_description} {joined_string} \n")
      else:
        lestaches.write(line)
    if search is False:
      print(f"la tache d'identifiant {id} est introuvable")
...
def show_tasks(filename):
  with open(filename, 'r') as lestaches:
    ...
    print('|  ' + 'id' + ' |' + 'description' + (taille - 11) * ' ' + '|'+ 'contexte' + (taille - 8) * ' ' + '|')
    print('+' + 5 * '-' + '+' + taille * '-'  + '+' + taille * '-' + '+')
    for line in lines:
      id= str(line.strip().split()[0])
      contexte=str(line.strip().split()[-1])
      descrip= line[1:line.find(contexte)]
      print('|  ' + id + '  |' + descrip + (taille - len(descrip)) * ' ' + '|'+ contexte + (taille - len(contexte) )* ' ' + '|')
      print('+' + 5 * '-' + '+' + taille * '-'  + '+' + taille * '-' + '+')


def main():
  parser = argparse.ArgumentParser()
  parser.add_argument('filename', help='Nom du fichier des tâches')
  ...
  modicont_parser = subparsers.add_parser('modicont', help="Modifier le contexte d'une tâche")
